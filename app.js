...
function drawSeriesVoltageDiagram(root,components,R,X,U,I,phi,f){
  ...
  if(Math.abs(UX)>.0001)line(root,rx,y0,rx,finalY,'vector','#ff9f43');line(root,x0,finalY,rx,finalY,'guide');const uEnd=arrow(root,x0,y0,U*safeS,deg(Math.atan2(UX,URsum||.000001)),VOLTAGE_COLOR,'',.8);text(root,uEnd.x+12,uEnd.y-12,`U = ${da(U,0)} V`,'vector-label',VOLTAGE_COLOR);text(root,50,330,'R-komponenter ligger på x-aksen; reaktanserne tegnes lodret fra R-summen.','vector-label','#778791');
}
