...
function drawThreeLoadPhaseFocus(root,analysis){
  ...
  const phase=clamp(threeLoadFocusPhase,1,3),phaseIndex=phase-1,refs=[90,-30,-150],ref=refs[phaseIndex],cx=235,cy=165,W=520,H=285,voltageNames=['U₁₂ / U₁₀','U₂₃ / U₂₀','U₃₁ / U₃₀'];
  drawAxisGrid(root,cx,cy,W,H);line(root,cx,cy,cx,46,'vector',VOLTAGE_COLOR);text(root,cx+12,60,`${voltageNames[phaseIndex]} som lokal reference`,'vector-label',VOLTAGE_COLOR,'start');
  ...
  text(root,cx-28,cy+31,'φ','vector-label','#8c9aa3','middle');
  if(showBranches)phaseParts.forEach((part,k)=>{
    const len=Math.max(24,part.value*scale*.82),angle=localAngle(part),end=arrow(root,cx,cy,len,angle,part.color,'',.52),anchor=end.x<cx?'end':'start';
    text(root,end.x+(anchor==='end'?-10:10),end.y+(k%2?-9:15),part.name,'vector-label',part.color,anchor);
  });
  if(total){
    const len=Math.max(38,total.value*scale),angle=localAngle(total),end=arrow(root,cx,cy,len,angle,CURRENT_COLOR,'',.82),anchor=end.x<cx?'end':'start',phi=localPhi(total);
    text(root,end.x+(anchor==='end'?-10:10),end.y-10,total.name,'vector-label',CURRENT_COLOR,anchor);
    if(Math.abs(phi)>.2){const r=32,from={x:cx,y:cy-r},to={x:cx+r*Math.cos(rad(angle)),y:cy-r*Math.sin(rad(angle))},sweep=phi>0?1:0;root.append(svg('path',{d:`M ${from.x} ${from.y} A ${r} ${r} 0 0 ${sweep} ${to.x} ${to.y}`,class:'arc'}));text(root,cx+(phi>0?38:-38),cy-28,`φ = ${da(Math.abs(phi),1)}°`,'vector-label','#8c9aa3','middle');}
  }
  if(neutral&&neutral.value>.01){
    const nPhi=normalizeAngle(90-neutral.angle),nAngle=90-nPhi,nEnd=arrow(root,cx,cy,Math.max(28,neutral.value*scale*.75),nAngle,CURRENT_COLOR,'',.55),nAnchor=nEnd.x<cx?'end':'start';
    text(root,nEnd.x+(nAnchor==='end'?-10:10),nEnd.y+17,'I₀','vector-label',CURRENT_COLOR,nAnchor);
  }
  const values=[...visible,neutral].filter(Boolean),lx=500,ly=60,row=18;root.append(svg('rect',{x:lx-14,y:ly-18,width:192,height:38+row*values.length,rx:8,fill:themeSurface(),stroke:'#263946','stroke-width':1}));text(root,lx,ly,`Værdier for fase ${phase}`,'parallel-state','#8f9da6');
  ...
  text(root,640,288,showBranches?'Fasebidrag og resultant vises lokalt.':'Kun den samlede netstrøm vises lokalt.','vector-label','#8f9da6','end');
  $('threeLoadFocusTitle').textContent=`Fase ${phase}: lokal vektorvisning`;
  $('threeLoadFocusNote').textContent=`Alle vinkler er målt fra fase ${phase}s egen spændingsreference. I₀ måles fra lodret nulreference.`;
}
