...
function drawThreeLoadPhaseFocus(root,analysis){
  ...
  const values=visible,lx=650,ly=66,row=20;root.append(svg('rect',{x:lx-16,y:ly-20,width:240,height:82+row*values.length,rx:8,fill:themeSurface(),stroke:'#263946','stroke-width':1}));text(root,lx,ly,`Værdier for fase ${phase}`,'parallel-state','#8f9da6');text(root,lx,ly+18,`Reference: ${referenceNames[phaseIndex]}`,'parallel-state',VOLTAGE_COLOR);text(root,lx,ly+36,`φ = ${da(focusPhi,1)}° fra lokal U-reference`,'parallel-state','#8f9da6');
  values.forEach((b,k)=>{const y=ly+62+k*row,angle=localPhi(b),c=b.name.includes('.')?b.color:CURRENT_COLOR;root.append(svg('circle',{cx:lx-8,cy:y-4,r:3,fill:c}));text(root,lx,y,`${b.name} ${da(b.value,2)} A ∠${da(angle,0)}°`,'vector-label',c);});
