...
function threeLoadAsymAnalysis(Uf,Un){
  ...
  groups.forEach((group,gIndex)=>{
    ...
    const b=group.branches.map((branch,i)=>{const value=branch.active?sourceU/branch.Zmag:0,angle=refs[i]+branch.zAngle,vec=vectorFrom(value,angle),item={...branch,group:group.index,connection:group.connection,name:branch.currentName,value,angle,globalAngle:angle,branchPhi:branch.zAngle,localAngle:group.connection==='Y'?localFromPhase(angle,i):null,vec,ref:refs[i],refName:(group.connection==='Y'?phaseRefNames:deltaRefNames)[i],color:colors[gIndex%colors.length]};if(branch.active){P+=sourceU*value*Math.cos(rad(branch.zAngle));Q-=sourceU*value*Math.sin(rad(branch.zAngle));branches.push(item);}return item;});
...
function drawThreeLoad(){
  ...
  $('threeLoadExplainer').innerHTML=asym?`<strong>Usymmetrisk</strong><p>Hver belastningsgruppe vælger Y eller Δ.<br>${phaseFormula}; Δ-grene bruger indtastet U<sub>n</sub> = ${da(Un,1)} V.<br>Y-reference: U<sub>10</sub>, U<sub>20</sub>, U<sub>30</sub>. Δ-reference: U<sub>12</sub>, U<sub>23</sub>, U<sub>31</sub>.<br>Vinkel φ indtastes som strømmens vinkel i forhold til grenens spænding.<br>I fasevisning omregnes Δ-grene til U<sub>10</sub>, U<sub>20</sub> eller U<sub>30</sub>. Eksempel: I<sub>12</sub> vises som 30° + φ i forhold til U<sub>10</sub>, mens I<sub>n1</sub> er vektorforskellen I<sub>12</sub> − I<sub>31</sub>.</p>`:threeLoadConnection==='Y'?`<strong>Y</strong><p>Indtastet U<sub>f</sub> = ${da(Uf,1)} V<br>I<sub>n</sub> = I<sub>f</sub><br>Z<sub>eq</sub>: R = ${da(R,1)} Ω, X = ${da(X,1)} Ω</p>`:`<strong>Δ</strong><p>Fasebelastningen ligger mellem to faser, så U<sub>f</sub> = U<sub>n</sub> = ${da(Un,1)} V<br>I<sub>n</sub> = √3 · I<sub>f</sub><br>Z<sub>eq</sub>: R = ${da(R,1)} Ω, X = ${da(X,1)} Ω</p>`;
