...
function drawIronPowerTriangle(root,PCu,PFe,P,Q,S,phi){
  if(!root)return;clear(root);const A={x:82,y:250},base=555,height=205,split=clamp(base*(PCu/Math.max(P,.001)),80,base-80),B={x:A.x+base,y:A.y},C={x:B.x,y:A.y-height};
  ...
  text(root,A.x+base*.5,A.y-height*.58,`S = ${apparentPower(S)}`,'vector-label',VOLTAGE_COLOR,'middle');text(root,B.x+20,A.y-height*.48,`Q = ${power(Q,true)}`,'vector-label','#ff9f43');text(root,A.x+split*.5,A.y+30,`P_Cu = ${power(PCu)}`,'vector-label','#9b87f5','middle');text(root,A.x+split+(base-split)/2,A.y+30,`P_Fe = ${power(PFe)}`,'vector-label','#55d6a1','middle');text(root,A.x+38,A.y-10,`φ = ${da(phi,1)}°`,'vector-label','#aab8c0','middle');
  text(root,A.x+base*.5,A.y+57,`P = ${power(P)}`,'triangle-dimension','#71828d','middle');text(root,A.x+split,A.y+14,'|','triangle-dimension','#71828d','middle');
}
function drawIronVectorDiagram(root,I,RCu,RFe,Xeff,Reff,Z,phi){
  if(!root)return;clear(root);
  const left={x:82,y:268},ur=96,ufe=68,ux=185,right={x:545,y:268},w=165,h=185,rcuW=70,rfeW=95;
  ...
  line(root,right.x,right.y,right.x+w,right.y,'vector','#d6e1e5');line(root,right.x+w,right.y,right.x+w,right.y-h,'vector','#d6e1e5');line(root,right.x,right.y,right.x+w,right.y-h,'vector','#d6e1e5');line(root,right.x+rcuW,right.y,right.x+rcuW,right.y+12,'axis');root.append(svg('path',{d:`M ${right.x+w-22} ${right.y} L ${right.x+w-22} ${right.y-22} L ${right.x+w} ${right.y-22}`,class:'triangle-right-angle'}));
  text(root,right.x+w*.5,right.y-h*.5-13,`Z = ${da(Z,2)} Ω`,'vector-label',VOLTAGE_COLOR,'middle');text(root,right.x+rcuW*.5,right.y+29,'R_Cu','vector-label','#9b87f5','middle');text(root,right.x+rcuW+rfeW*.5,right.y+29,'R_Fe','vector-label','#55d6a1','middle');text(root,right.x+w*.5,right.y+54,`R_eff = ${da(Reff,2)} Ω`,'vector-label','#d6e1e5','middle');text(root,right.x+w+14,right.y-h*.5,`X_L,eff = ${da(Xeff,2)} Ω`,'vector-label','#ff9f43');text(root,right.x+25,right.y-9,'φ','vector-label','#aab8c0');
  text(root,192,44,'Vektordiagram','triangle-dimension','#71828d','middle');text(root,638,44,'Impedanstrekant','triangle-dimension','#71828d','middle');
}
function drawIronHysteresis(root){
  ...
  for(let h=-2000;h<=2000;h+=500)line(root,toX(h),58,toX(h),388,'grid-line');for(let b=-1.5;b<=1.51;b+=.25)line(root,185,toY(b),700,toY(b),'grid-line');
  line(root,170,cy,727,cy,'axis');line(root,cx,50,cx,396,'axis');arrow(root,727,cy,35,0,'#d6e1e5','',.55);arrow(root,cx,50,35,90,'#d6e1e5','',.55);text(root,766,cy-8,'H','vector-label','#d6e1e5');text(root,cx+9,43,'B [T]','vector-label','#d6e1e5');text(root,740,cy+20,'[Avind/m]','triangle-dimension','#aab8c0');
