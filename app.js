...
function drawRlc(){
  const {U,f,R,L,C,XL,XC,components}=seriesValues(),X=XL-XC,Z=Math.hypot(R,X),I=Z?U/Z:0,phi=deg(Math.atan2(X,R)),P=I*I*R,hasL=XL>.00001,hasC=XC>.00001,isResonant=hasL&&hasC&&Math.abs(X)<.05;drawSeriesCircuit(components);
  ...
  $('rlcXL').textContent=unit(XL,'Ω');$('rlcXC').textContent=unit(XC,'Ω');$('rlcZ').textContent=unit(Z,'Ω');$('rlcI').textContent=unit(I,'A',2);$('rlcPhi').textContent=unit(phi,'°');$('rlcP').textContent=power(P);$('rlcPhiText').textContent=`φ = ${da(phi,1)}°`;$('rlcNature').textContent=isResonant?'Resonans · spænding og strøm i fase':Math.abs(X)<.05?'Resistiv · spænding og strøm i fase':X>0?'Induktiv · strømmen halter efter':'Kapacitiv · strømmen er foran';
...
function drawSeriesUIPhasor(U,I,phi){
  const root=$('seriesUIPhasor'),cx=260,cy=190,iAngle=90-phi;clear(root);baseGrid(root,cx,cy,480,240);root.append(svg('circle',{cx,cy,r:110,class:'guide'}));
  arrow(root,cx,cy,145,90,VOLTAGE_COLOR,`U = ${da(U,0)} V`,.85);arrow(root,cx,cy,125,iAngle,CURRENT_COLOR,`I = ${da(I,2)} A`,.85);
  if(Math.abs(phi)>.05){const r=42,sweep=phi>0?1:0,endX=cx+r*Math.cos(rad(iAngle)),endY=cy-r*Math.sin(rad(iAngle)),midAngle=90-phi/2;root.append(svg('path',{d:`M ${cx} ${cy-r} A ${r} ${r} 0 0 ${sweep} ${endX} ${endY}`,class:'arc'}));text(root,cx+58*Math.cos(rad(midAngle)),cy-58*Math.sin(rad(midAngle))+4,`φ ${da(Math.abs(phi),1)}°`,'vector-label','#778791','middle');}
  const currentAngle=-phi;$('seriesUIAngle').textContent=`I∠${Math.abs(currentAngle)<.05?'0':currentAngle>0?'+'+da(currentAngle,1):da(currentAngle,1)}°`;
}
...
function drawImpedanceTriangle(R,XL,XC,X,Z,phi){
  ...
  $('impedancePhiFormula').textContent=R>0?`φ = tan⁻¹(${da(X,1)} / ${da(R,1)}) = ${da(phi,1)}°`:`φ = ${da(phi,1)}°`;
  $('impedanceCosFormula').textContent=Z>0?`cos φ = ${da(R,1)} / ${da(Z,1)} = ${da(R/Z,3)}`:'cos φ = —';
  ...
  if(Math.abs(phi)>1){const radius=38,sweep=phi>0?0:1,endX=x0+radius*Math.cos(rad(phi)),endY=y0-radius*Math.sin(rad(phi));root.append(svg('path',{d:`M ${x0+radius} ${y0} A ${radius} ${radius} 0 0 ${sweep} ${endX} ${endY}`,class:'arc'}));text(root,x0+50,y0+(phi>0?-9:31),`φ ${da(phi,1)}°`,'vector-label','#83919a');}
...
function drawCompCurrentPhasor(Ip,Iq1,Iq2,I1,I2,Ic){
  const root=$('compCurrentPhasor'),cx=120,cy=235,maxQ=Math.max(Iq1,Iq2,Ic,.001),scale=Math.min(Ip>0?145/Ip:Infinity,245/maxQ),s=Number.isFinite(scale)?scale:1,tip1={x:cx+Iq1*s,y:cy-Ip*s},tip2={x:cx+Iq2*s,y:cy-Ip*s};clear(root);baseGrid(root,cx,cy,480,240);arrow(root,cx,cy,165,90,'#526570','U · reference',.72);const a1=deg(Math.atan2(Ip,Iq1||.000001)),a2=deg(Math.atan2(Ip,Iq2||.000001));arrow(root,cx,cy,I1*s,a1,'#667581','',.75);text(root,tip1.x+12,tip1.y-10,`I₁ ${da(I1,2)} A`,'vector-label','#8a98a2');arrow(root,cx,cy,I2*s,a2,'#35d3e3','',.8);text(root,tip2.x-8,tip2.y+20,`I₂ ${da(I2,2)} A`,'vector-label','#35d3e3','end');if(Ic>.001){arrow(root,tip1.x,tip1.y,Ic*s,180,'#55d6a1','',.7);text(root,(tip1.x+tip2.x)/2,tip1.y-12,`Iᴄ ${da(Ic,2)} A`,'vector-label','#55d6a1','middle');}line(root,cx,tip1.y,tip1.x,tip1.y,'guide');text(root,465,283,'I₂ = I₁ + Iᴄ','vector-label','#778690','end');
}

function drawCompCurrentFlow(Ibefore,Iafter,Ic,U,valid){
  const root=$('compCurrentFlow');clear(root);root.append(svg('rect',{x:45,y:76,width:100,height:62,rx:9,class:'circuit-component'}));text(root,95,101,'FORSYNING','vector-label','#8c9aa3','middle');text(root,95,121,`Uₙ ${da(U,0)} V`,'vector-label','#35d3e3','middle');circuitWire(root,145,107,755,107,'#71808a');root.append(svg('rect',{x:755,y:65,width:110,height:84,rx:9,class:'circuit-component'}));text(root,810,91,'BELASTNING','vector-label','#d3dde1','middle');text(root,810,112,`I ${da(Ibefore,2)} A`,'vector-label','#ff9f43','middle');text(root,810,131,'før komp.','vector-label','#70808b','middle');circuitArrow(root,195,107,130,0,`Iₙ efter ${da(Iafter,2)} A`,'#35d3e3');circuitArrow(root,575,107,130,0,`I belastning ${da(Ibefore,2)} A`,'#ff9f43');circuitNode(root,505,107);circuitWire(root,505,107,505,214,'#55d6a1');circuitArrow(root,505,122,38,-90,`Iᴄ ${da(Ic,2)} A`,'#55d6a1');circuitCapacitor(root,505,196,90);text(root,548,193,'KONDENSATORBATTERI','vector-label','#55d6a1');text(root,548,213,valid?'Kapacitiv grenstrøm':'Kontrollér inddata','vector-label','#81909a');text(root,450,244,'Kondensatorstrømmen leverer den kapacitive reaktive komponent lokalt','vector-label','#667681','middle');
}
