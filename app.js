...
function drawThreeLoadPhasor(root,connection,phi,If,In,Uf=0,parts=[],analysis=null){
  ...
  pts.forEach((p,i)=>{
    const ref=refs[i],deltaRef=ref+30,lineName=isDelta?deltaCurrentNames[i]:`Iₙ${i+1}`,currentRef=isDelta?deltaRef:ref,currentAngle=showComponentCurrents?currentRef+branchPhi:currentRef-phi,shownValue=isDelta?If:In,refEnd={x:p[0]+54*Math.cos(rad(currentRef)),y:p[1]-54*Math.sin(rad(currentRef))};
    root.append(svg('line',{x1:p[0],y1:p[1],x2:refEnd.x,y2:refEnd.y,class:'guide',stroke:'#8a98a3','stroke-dasharray':'7 6'}));
    if(showComponentCurrents){const maxI=Math.max(branchTotal,...branchCurrents.map(b=>b.value),.001),baseLen=52;branchCurrents.forEach(b=>arrow(root,p[0],p[1],Math.max(17,b.value/maxI*baseLen),currentRef+b.angle,b.color,'',.4));}
    ...
    const shownPhi=isDelta?normalizeAngle(currentAngle-ref):normalizeAngle(currentAngle-currentRef),phiReference=isDelta?ref:currentRef;if(Math.abs(shownPhi)>.2){const r=24,from={x:p[0]+r*Math.cos(rad(phiReference)),y:p[1]-r*Math.sin(rad(phiReference))},to={x:p[0]+r*Math.cos(rad(currentAngle)),y:p[1]-r*Math.sin(rad(currentAngle))},sweep=isDelta?(shownPhi>0?0:1):(shownPhi>0?1:0);root.append(svg('path',{d:`M ${from.x} ${from.y} A ${r} ${r} 0 0 ${sweep} ${to.x} ${to.y}`,class:'arc'}));text(root,p[0]+(i===1?36:-30),p[1]+(i===0?-18:20),`φ = ${da(shownPhi,1)}°`,'vector-label','#8c9aa3','middle');}
  });
  if(showComponentCurrents){
    const lx=500,ly=55,row=18;root.append(svg('rect',{x:lx-14,y:ly-18,width:178,height:38+row*(branchCurrents.length+1),rx:8,fill:themeSurface(),stroke:'#263946','stroke-width':1}));text(root,lx,ly,'Værdier i én fase','parallel-state','#8f9da6');branchCurrents.forEach((b,k)=>{const y=ly+24+k*row;root.append(svg('circle',{cx:lx-8,cy:y-4,r:3,fill:b.color}));text(root,lx,y,`${b.name} ${da(b.value,2)} A ∠${da(b.angle,0)}°`,'vector-label',b.color);});const y=ly+24+branchCurrents.length*row;root.append(svg('circle',{cx:lx-8,cy:y-4,r:3,fill:CURRENT_COLOR}));text(root,lx,y,isDelta?`I₁₂ ${da(branchTotal,2)} A`:`Iₙ1 ${da(branchTotal,2)} A`,'vector-label',CURRENT_COLOR);
  }
  text(root,640,365,showComponentCurrents?(isDelta?'Rød resultant er grenstrømmen i den viste Δ-side.':'Rød resultant er netstrømmen Iₙ.'):connection==='D'?`Δ: rød vektor er I_z. Iₙ = √3 · I_z = ${da(In,2)} A`:`Y: Iₙ = I_z = ${da(In,2)} A`,'vector-label','#8f9da6','end');
}
