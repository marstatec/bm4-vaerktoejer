...
function threeLoadAsymAnalysis(Uf,Un){
  ...
  groups.forEach((group,gIndex)=>{
    ...
    const b=group.branches.map((branch,i)=>{const value=branch.active?sourceU/branch.Zmag:0,currentAngle=-branch.zAngle,angle=refs[i]+currentAngle,vec=vectorFrom(value,angle),item={...branch,group:group.index,connection:group.connection,name:branch.currentName,value,angle,globalAngle:angle,zPhi:branch.zAngle,branchPhi:currentAngle,localAngle:group.connection==='Y'?localFromPhase(angle,i):null,vec,ref:refs[i],refName:(group.connection==='Y'?phaseRefNames:deltaRefNames)[i],color:colors[gIndex%colors.length]};if(branch.active){P+=sourceU*value*Math.cos(rad(branch.zAngle));Q+=sourceU*value*Math.sin(rad(branch.zAngle));branches.push(item);}return item;});
...
function drawThreeLoad(){
  ...
  $('threeLoadExplainer').innerHTML=asym?`<strong>Usymmetrisk</strong><p>Hver belastningsgruppe vælger Y eller Δ.<br>${phaseFormula}; Δ-grene bruger indtastet U<sub>n</sub> = ${da(Un,1)} V.<br>Y-reference: U<sub>10</sub>, U<sub>20</sub>, U<sub>30</sub>. Δ-reference: U<sub>12</sub>, U<sub>23</sub>, U<sub>31</sub>.<br>Vinkel φ indtastes som impedansvinklen ∠Z. Strømmen beregnes derfor som I = U/Z og får modsat fortegn.<br>I fasevisning omregnes Δ-grene til U<sub>10</sub>, U<sub>20</sub> eller U<sub>30</sub>. Eksempel: I<sub>12</sub> vises som 30° − φ<sub>Z</sub> i forhold til U<sub>10</sub>, mens I<sub>n1</sub> er vektorforskellen I<sub>12</sub> − I<sub>31</sub>.</p>`:threeLoadConnection==='Y'?`<strong>Y</strong><p>Indtastet U<sub>f</sub> = ${da(Uf,1)} V<br>I<sub>n</sub> = I<sub>f</sub><br>Z<sub>eq</sub>: R = ${da(R,1)} Ω, X = ${da(X,1)} Ω</p>`:`<strong>Δ</strong><p>Fasebelastningen ligger mellem to faser, så U<sub>f</sub> = U<sub>n</sub> = ${da(Un,1)} V<br>I<sub>n</sub> = √3 · I<sub>f</sub><br>Z<sub>eq</sub>: R = ${da(R,1)} Ω, X = ${da(X,1)} Ω</p>`;
...
function drawThreeLoadPhasor(root,connection,phi,If,In,Uf=0,parts=[],analysis=null){
  ...
  if(analysis?.asym){
    ...
    const lineContribs=(analysis.groupLines||[]).map(b=>({...b,valueKind:'bidrag'})),lineTotals=(analysis.lines||[]).map(b=>({...b,valueKind:'net'})),neutralValue=!analysis.deltaOnly&&analysis.neutral?[{...analysis.neutral,valueKind:'nul'}]:[],values=showBranches?[...lineContribs,...lineTotals,...neutralValue].filter(Boolean):[...lineTotals,...neutralValue].filter(Boolean),lx=556,ly=58,row=15,commonRef=phaseRefs[0],cols=showBranches&&values.length>13?2:1,rows=Math.ceil(values.length/cols),displayAngle=b=>{const angle=Number.isFinite(b.globalAngle)?b.globalAngle:b.angle;if(b.name==='I₀'&&b.value<.01)return 0;return normalizeAngle(angle-commonRef);},kindLabel=b=>b.valueKind==='bidrag'?' bidrag':'';root.append(svg('rect',{x:lx-14,y:ly-18,width:cols===2?344:230,height:38+row*rows,rx:8,fill:themeSurface(),stroke:'#263946','stroke-width':1}));text(root,lx,ly,showBranches?'Alle netstrømsbidrag · ref. U₁₀':'Samlede netstrømme · ref. U₁₀','parallel-state','#8f9da6');values.forEach((b,k)=>{const col=cols===2&&k>=rows?1:0,r=col?k-rows:k,x=lx+col*168,y=ly+22+r*row,c=b.valueKind==='net'||b.valueKind==='nul'?CURRENT_COLOR:b.color;root.append(svg('circle',{cx:x-8,cy:y-4,r:3,fill:c}));text(root,x,y,`${b.name}${kindLabel(b)} ${da(b.value,2)} A ∠${da(displayAngle(b),0)}°`,'vector-label',c);});
...
function drawThreeLoadPhaseFocus(root,analysis){
  ...
  const values=visible,lx=626,ly=66,row=24;root.append(svg('rect',{x:lx-16,y:ly-20,width:300,height:102+row*values.length,rx:8,fill:themeSurface(),stroke:'#263946','stroke-width':1}));text(root,lx,ly,`Værdier for fase ${phase}`,'parallel-state','#8f9da6');text(root,lx,ly+18,`Visningsreference: ${referenceName}`,'parallel-state',VOLTAGE_COLOR);text(root,lx,ly+36,'Y beregnes med U₁₀/U₂₀/U₃₀','parallel-state','#8f9da6');text(root,lx,ly+54,'Δ beregnes med U₁₂/U₂₃/U₃₁','parallel-state','#8f9da6');
  values.forEach((b,k)=>{const y=ly+82+k*row,angle=localPhi(b),c=b.name.includes('.')?b.color:CURRENT_COLOR;root.append(svg('circle',{cx:lx-8,cy:y-4,r:3,fill:c}));text(root,lx,y,`${b.name} ${da(b.value,2)} A ∠${da(angle,0)}°`,'vector-label',c);});
  $('threeLoadFocusTitle').textContent=`Fase ${phase}: zoom på strømvektorer`;
  $('threeLoadFocusNote').textContent=`Lokal visningsreference: ${referenceName}. Δ-grene beregnes med fase-fase-spænding, og strømvinklen omregnes til denne fase.`;
}
...
function renderThreeLoadImpedanceFields(){
  ...
  const rows=Array.from({length:THREE_LOAD_MAX_COMPONENTS},(_,i)=>{
    ...
    const cells=[0,1,2].map(p=>{
      ...
      return `<div class="impedance-phase-cell"><div class="impedance-cell-head"><span>Z<sub>${sub}</sub> · ${names[p]}</span><label class="impedance-enable"><input id="threeLoadActivec${c}p${p+1}" type="checkbox"${checked}> Til</label></div><input id="threeLoadZc${c}p${p+1}" type="number" value="${z}" min="0" step="0.1" placeholder="frakoblet" aria-label="Impedans Z${c}${subs[p]} i ohm"><input id="threeLoadPhic${c}p${p+1}" type="number" value="${phi}" min="-89.9" max="89.9" step="0.1" placeholder="∠Z" aria-label="Impedansvinkel phi ${c}${subs[p]} i grader"></div>`;
    }).join('');
