...
function drawThreeLoadPhasor(root,connection,phi,If,In,Uf=0,parts=[],analysis=null){
  ...
  if(analysis?.asym){
    const maxI=Math.max(...analysis.groupLines.map(b=>b.value),...analysis.lines.map(l=>l.value),analysis.neutral?.value||0,.001),lineLen=96,branchLen=70,phaseRefs=[90,-30,-150];
    pts.forEach((p,i)=>{
      const ref=phaseRefs[i],refEnd={x:p[0]+54*Math.cos(rad(ref)),y:p[1]-54*Math.sin(rad(ref))},lineCurrent=analysis.lines[i],phaseParts=analysis.groupLines.filter(g=>g.phase===i+1);
    ...
    const branchValues=(analysis.branches||[]).map(b=>({...b,valueKind:'gren'})),lineContribs=(analysis.groupLines||[]).map(b=>({...b,valueKind:'bidrag'})),lineTotals=(analysis.lines||[]).map(b=>({...b,valueKind:'net'})),neutralValue=analysis.neutral?[{...analysis.neutral,valueKind:'nul'}]:[],values=showBranches?[...branchValues,...lineContribs,...lineTotals,...neutralValue].filter(Boolean):[...lineTotals,...neutralValue].filter(Boolean),lx=556,ly=58,row=15,commonRef=phaseRefs[0],cols=showBranches&&values.length>13?2:1,rows=Math.ceil(values.length/cols),displayAngle=b=>{const angle=Number.isFinite(b.globalAngle)?b.globalAngle:b.angle;return b.name==='I₀'&&b.value<.01?0:normalizeAngle(angle-commonRef);},kindLabel=b=>b.valueKind==='gren'?' gren':b.valueKind==='bidrag'?' bidrag':'';root.append(svg('rect',{x:lx-14,y:ly-18,width:cols===2?344:230,height:38+row*rows,rx:8,fill:themeSurface(),stroke:'#263946','stroke-width':1}));text(root,lx,ly,showBranches?'Alle strømme · vist ift. U₁₀':'Samlede netstrømme · ref. U₁₀','parallel-state','#8f9da6');values.forEach((b,k)=>{const col=cols===2&&k>=rows?1:0,r=col?k-rows:k,x=lx+col*168,y=ly+22+r*row,c=b.valueKind==='net'||b.valueKind==='nul'?CURRENT_COLOR:b.color;root.append(svg('circle',{cx:x-8,cy:y-4,r:3,fill:c}));text(root,x,y,`${b.name}${kindLabel(b)} ${da(b.value,2)} A ∠${da(displayAngle(b),0)}°`,'vector-label',c);});
  ...
  pts.forEach((p,i)=>{
    ...
    const shownPhi=isDelta?normalizeAngle(currentAngle-ref):(showComponentCurrents?-branchPhi:phi),phiReference=isDelta?ref:currentRef;if(Math.abs(shownPhi)>.2){const r=24,from={x:p[0]+r*Math.cos(rad(phiReference)),y:p[1]-r*Math.sin(rad(phiReference))},to={x:p[0]+r*Math.cos(rad(currentAngle)),y:p[1]-r*Math.sin(rad(currentAngle))},sweep=isDelta?(shownPhi>0?0:1):(shownPhi>0?1:0);root.append(svg('path',{d:`M ${from.x} ${from.y} A ${r} ${r} 0 0 ${sweep} ${to.x} ${to.y}`,class:'arc'}));text(root,p[0]+(i===1?36:-30),p[1]+(i===0?-18:20),`φ = ${da(isDelta?shownPhi:Math.abs(shownPhi),1)}°`,'vector-label','#8c9aa3','middle');}
  });
