...
function drawThreeLoad(){
  ...
  $('threeLoadDiagramTitle').textContent=asymDeltaOnly?'Usymmetrisk Δ: belastningsstrømme på spændingstrekant':mixed?'Usymmetrisk: samlede netstrømme':threeLoadConnection==='Y'?'Stjerne: netstrømme på spændingstrekant':'Trekant: belastningsstrømme på spændingstrekant';$('threeLoadDiagramNote').textContent=asymDeltaOnly?'Røde vektorer er de samlede Δ-grenstrømme I₁₂, I₂₃ og I₃₁. Netstrømmene beregnes stadig i resultaterne.':asym?'Røde vektorer er de samlede netstrømme Iₙ1, Iₙ2 og Iₙ3.':threeLoadConnection==='D'?'Røde vektorer er grenstrømmene I₁₂, I₂₃ og I₃₁. Netstrømmen Iₙ står i resultaterne.':'Røde vektorer er netstrømmene Iₙ1, Iₙ2 og Iₙ3.';$('threeLoadType').textContent=Q<0?'Kapacitiv: strømmen er foran':Q>0?'Induktiv: strømmen er bagefter':'Resistiv: strøm og spænding i fase';
...
function drawThreeLoadPhasor(root,connection,phi,If,In,Uf=0,parts=[],analysis=null){
  ...
  if(analysis?.asym){
    const displayTotals=analysis.deltaOnly&&!showBranches?analysis.deltaTotals:analysis.lines;
    ...
    const branchValues=(analysis.branches||[]).map(b=>({...b,valueKind:'gren'})),lineContribs=(analysis.groupLines||[]).map(b=>({...b,valueKind:'bidrag'})),lineTotals=(analysis.lines||[]).map(b=>({...b,valueKind:'net'})),deltaTotals=(analysis.deltaTotals||[]).map(b=>({...b,valueKind:'deltaTotal'})),neutralValue=!analysis.deltaOnly&&analysis.neutral?[{...analysis.neutral,valueKind:'nul'}]:[],values=showBranches?[...branchValues,...lineContribs,...lineTotals,...neutralValue].filter(Boolean):[...(analysis.deltaOnly?deltaTotals:lineTotals),...neutralValue].filter(Boolean),lx=556,ly=58,row=15,commonRef=phaseRefs[0],cols=showBranches&&values.length>13?2:1,rows=Math.ceil(values.length/cols),displayAngle=b=>{const angle=Number.isFinite(b.globalAngle)?b.globalAngle:b.angle;if(b.name==='I₀'&&b.value<.01)return 0;if(b.valueKind==='deltaTotal')return normalizeAngle(angle-phaseRefs[(b.phase||1)-1]);return normalizeAngle(angle-commonRef);},kindLabel=b=>b.valueKind==='gren'?' gren':b.valueKind==='bidrag'?' bidrag':'';root.append(svg('rect',{x:lx-14,y:ly-18,width:cols===2?344:230,height:38+row*rows,rx:8,fill:themeSurface(),stroke:'#263946','stroke-width':1}));text(root,lx,ly,showBranches?'Alle strømme · vist ift. U₁₀':analysis.deltaOnly?'Δ-grenstrømme · lokal fase-reference':'Samlede netstrømme · ref. U₁₀','parallel-state','#8f9da6');values.forEach((b,k)=>{const col=cols===2&&k>=rows?1:0,r=col?k-rows:k,x=lx+col*168,y=ly+22+r*row,c=b.valueKind==='net'||b.valueKind==='nul'||b.valueKind==='deltaTotal'?CURRENT_COLOR:b.color;root.append(svg('circle',{cx:x-8,cy:y-4,r:3,fill:c}));text(root,x,y,`${b.name}${kindLabel(b)} ${da(b.value,2)} A ∠${da(displayAngle(b),0)}°`,'vector-label',c);});
    text(root,640,365,analysis.deltaOnly?'Δ-grenstrømme vises på spændingstrekanten; netstrømme beregnes i resultaterne.':connection==='M'?'Usymmetrisk: I₀ er nulstrømmen fra Y-grupperne.':connection==='D'?'Iₙ beregnes som vektorforskel: Iₙ1 = I₁₂ − I₃₁.':'Iₙ er den enkelte fasegrenstrøm i stjerne.','vector-label','#8f9da6','end');return;
  }
